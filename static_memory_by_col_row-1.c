...
//===================================Work Functions===================================
// the actual work
// my matrices are manually stored using column major (in memory col1 values -> col 2 values -> col 3 values)
void multiplyByColumn(int col) {
    for (int j = 0; j < dim; j++) {
        product_matrix[col * dim + j] = 0;
        for (int k = 0; k < dim; k++) {
            product_matrix[col * dim + j] += reference_matrix[col * dim + k] * original_matrix[k * dim + j];
        }
    }
}

// my matrices are manually stored using column major (in memory col1 values -> col 2 values -> col 3 values)
void multiplyByRow(int row) {
    for (int i = 0; i < dim; i++) {
        product_matrix[i * dim + row] = 0;
        for (int k = 0; k < dim; k++) {
            product_matrix[i * dim + row] += reference_matrix[k * dim + row] * original_matrix[i * dim + k];
        }
    }
}
        ...
        while (jobs_left > -1) {
            ...
            if (job >= 0) {
                if (row_or_col == 1) {
                    multiplyByRow(job);
                    if(dialogue == 2) {
                        printf("Thread %d has calculated row %d\n", worker_info->thread_no, job);
                    }
                } else {
                    multiplyByColumn(job);
                    if (dialogue == 2) {
                        printf("Thread %d has calculated column %d\n", worker_info->thread_no, job);
                    }
                }
            }
        }
...
int main() {
    ...
    // each job is either a product matrix row or product matrix column
    // so jobs_left = 99 would be row or column 99 in the product matrix
    jobs_left = dim - 1;
    job_iterations_left = power - 1;
