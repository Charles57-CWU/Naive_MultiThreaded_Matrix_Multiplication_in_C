...
void multiplyByColumn(struct Node* multiply_original, struct Node* multiply_product) {
    struct Node *old_ori_head = multiply_original;
    struct Node *old_ref_head = reference_head;
    struct Node *multiply_reference = old_ref_head;
    ...
    for (int j = 0; j < dim; j++) {
        ...
        multiply_product = multiply_product->down;
        multiply_original = old_ori_head;
        old_ref_head = old_ref_head->down;
        multiply_reference = old_ref_head;
    }
}

void multiplyByRow(struct Node* multiply_reference, struct Node* multiply_product) {
    struct Node *old_ref_head = multiply_reference;
    struct Node *old_ori_head = original_head;
    struct Node *multiply_original = old_ori_head;
    ...
    for (int i = 0; i < dim; i++) {
        ...
        multiply_product = multiply_product->right;
        multiply_reference = old_ref_head;
        old_ori_head = old_ori_head->right;
        multiply_original = old_ori_head;
    }
}
...
void threadWorkToDo(void*arg){
    ...
    while (job_iterations_left > 0) {
        ...
        while (jobs_left > 0) {
            pthread_mutex_lock(&job_countdown_mutex);
	    // if statement needed here if a thread makes it into the while loop but waiting on mutex
            // and the last row/col is being calculated to avoid out of bound indices
            if(jobs_left > 0) {
                if (row_or_col == 1) {
                    pthread_mutex_lock(&product_matrix_curr_mutex);
                    multiply_product = product_matrix_curr;
                    product_matrix_curr = product_matrix_curr->down;
                    pthread_mutex_unlock(&product_matrix_curr_mutex);
                    ...
                    multiplyByRow(multiply_reference, multiply_product);
                    if (dialogue == 2) {
                        printf("Thread %d is calculating row %d\n", worker_info->thread_no, jobs_left);
                    }
                } else {
                    pthread_mutex_lock(&product_matrix_curr_mutex);
                    multiply_product = product_matrix_curr;
                    ...
                    multiplyByColumn(multiply_original, multiply_product);
                    if (dialogue == 2) {
                        printf("Thread %d is calculating column %d\n", worker_info->thread_no, jobs_left);
                    }
                }
            } else {
                pthread_mutex_unlock(&job_countdown_mutex);
...
void getInputs() {
    ...
    *check;
    ...
    //get row or column choice
    printf("Please type 1 for by-row multiplication or 2 for by-column multiplication\n");
    fgets(line, BUFFER, stdin);
    if (strcmp(line, "\n") == 0) {
        printf("No input detected.\n");
        printf("Exiting Program.\n");
    ...
    *check;
    row_or_col = strtol(line, &check, 10);
    if (*check) {
        printf("Incorrect choice inputted.\n");
        printf("Choice must be 1 or 2\n");
        printf("Exiting Program.\n");
        exit(1);
    } else if (row_or_col!= 2 && row_or_col != 1) {
        printf("Incorrect choice inputted.\n");
        printf("Choice must be 1 or 2\n");
        printf("Exiting Program.\n");
        exit(1);
...
int main() {
    ...
    if (row_or_col == 1) {
        printf("Method: by-Row\n");
    } else {
        printf("Method: by-Column\n");
    }
